...
create_wordcloud <- function(data, stop_words = c(), num_words = 100, background = "white",  mask = NULL) {
  ...
  set.seed(90)
  wordcloud2(data, backgroundColor = background, color = "random-dark", fontFamily = "Asap", size = 1) 
}

# Generamos el wordcloud ----

wc <- create_wordcloud(tw$text, num_words = 200, stop_words = c("pueblo",
                                                          "vamos",
                                                          "votaporelcambio", 
                                                          "delfinagobernadora"))
saveWidget(wc, "wc.html")
...
names(accidentes_peatones)[1] <- "Date"
