---
title: "Tutorial Completo: Predicción de Precios de Inmuebles"
subtitle: "Desde Regresión Lineal hasta XGBoost"
...
## Teoría: Preprocesamiento de Datos
...
```{r filtrado}
precio_casas <- precio_casas %>%
  # NOTA: El filtro original tenía un error lógico
  # filter(is.na(m2_terreno)) filtraría SOLO los NA
  # Corrección: mantener los que SÍ tienen datos
  filter(!is.na(m2_terreno))
```

**Corrección importante**: El script original filtraba `is.na(m2_terreno)`, lo cual elimina datos válidos. Cambiamos a `!is.na()`.

### Conversión de Moneda y Transformaciones
...
### Filtrado de Ventas y Outliers

```{r filtrado-final}
precio_casas <- precio_casas %>%
  # Solo inmuebles en venta
...
# Verificar datos limpios (primeras 10 filas)
precio_casas %>%
...
print(paste("RMSE:", round(modelo_cv$results$RMSE[1], 4)))
print(paste("R²:", round(modelo_cv$results$Rsquared[1], 4)))

print("=== Modelo con log(precio) ===")
print(paste("RMSE:", round(modelo_log_cv$results$RMSE[1], 4)))
print(paste("R²:", round(modelo_log_cv$results$Rsquared[1], 4)))
```
...
# Mostrar resumen compacto del modelo
summary_lm <- summary(modelo_final)
print(paste("R² ajustado:", round(summary_lm$adj.r.squared, 4)))
print(paste("RMSE:", round(sqrt(mean(summary_lm$residuals^2)), 4)))
print("\nCoeficientes:")
print(tidy(modelo_final) %>% select(term, estimate, p.value))
```
...
print("=== Modelo Ridge ===")
print(paste("Mejor lambda:", round(modelo_ridge_cv$bestTune$lambda, 6)))
print(paste("RMSE:", round(min(modelo_ridge_cv$results$RMSE), 4)))
print(paste("R²:", round(max(modelo_ridge_cv$results$Rsquared), 4)))
```
...
print("=== Modelo Lasso ===")
print(paste("Mejor lambda:", round(modelo_lasso_cv$bestTune$lambda, 6)))
print(paste("RMSE:", round(min(modelo_lasso_cv$results$RMSE), 4)))
print(paste("R²:", round(max(modelo_lasso_cv$results$Rsquared), 4)))

# Ver coeficientes no cero (detectar cuáles fueron eliminados)
...
coef_lasso_df <- as.matrix(coef_lasso) %>%
  ...
  filter(coeficiente != 0)  # Solo mostrar coeficientes no cero

print("Coeficientes Lasso (solo variables seleccionadas):")
print(coef_lasso_df)
```

## 4. Árbol de Decisión

### Teoría: Árboles de Regresión
...
print(paste("Mejor cp:", round(modelo_arbol_cv$bestTune$cp, 6)))
print(paste("RMSE:", round(min(modelo_arbol_cv$results$RMSE), 4)))
print(paste("R²:", round(max(modelo_arbol_cv$results$Rsquared), 4)))
...
print(paste("Mejor mtry:", modelo_rf_cv$bestTune$mtry))
print(paste("RMSE:", round(min(modelo_rf_cv$results$RMSE), 4)))
print(paste("R²:", round(max(modelo_rf_cv$results$Rsquared), 4)))
...
### Implementación con Paralelización
...
print(paste("RMSE:", round(min(modelo_xgb_cv$results$RMSE), 4)))
print(paste("R²:", round(max(modelo_xgb_cv$results$Rsquared), 4)))

# Mejores hiperparámetros (solo los más relevantes)
print("\nMejores hiperparámetros:")
print(paste("  nrounds:", modelo_xgb_cv$bestTune$nrounds))
print(paste("  max_depth:", modelo_xgb_cv$bestTune$max_depth))
print(paste("  eta:", round(modelo_xgb_cv$bestTune$eta, 4)))
print(paste("  gamma:", round(modelo_xgb_cv$bestTune$gamma, 4)))
print(paste("  colsample_bytree:", round(modelo_xgb_cv$bestTune$colsample_bytree, 4)))
print(paste("  min_child_weight:", modelo_xgb_cv$bestTune$min_child_weight))
print(paste("  subsample:", round(modelo_xgb_cv$bestTune$subsample, 4)))
...
```{r tabla-tradeoffs, echo=FALSE}
tibble(
  Modelo = c("OLS", "Ridge/Lasso", "Árbol", "Random Forest", "XGBoost"),
  Interpretabilidad = c("★★★★★", "★★★★☆", "★★★☆☆", "★★☆☆☆", "★☆☆☆☆"),
  Velocidad_Entrenamiento = c("★★★★★", "★★★★☆", "★★★★☆", "★★☆☆☆", "★☆☆☆☆"),
  Velocidad_Predicción = c("★★★★★", "★★★★★", "★★★★☆", "★★★☆☆", "★★★☆☆"),
  Rendimiento_Predictivo = c("★★☆☆☆", "★★★☆☆", "★★★☆☆", "★★★★☆", "★★★★★"),
  Manejo_No_Linealidad = c("★☆☆☆☆", "★☆☆☆☆", "★★★★★", "★★★★★", "★★★★★")
) %>%
  knitr::kable(caption = "Comparación Cualitativa de Modelos")
```
